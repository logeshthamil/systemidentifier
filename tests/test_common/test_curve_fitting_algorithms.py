...
def test_sequenceofbiquads():
    ...
    ref_hgm = nlsp.HammersteinGroupModel(nonlinear_functions=nonlinear_function_ref,
                                         filter_impulseresponses=filter_impulse_responses_ref,
                                         aliasing_compensation=nlsp.aliasing_compensation.NoAliasingCompensation())
    curve_tracing_filters_iir, coeff = nlsp.curve_fitting_algorithms.compute_iir_from_fir_using_curvetracing_sequencialbiquads(fir_kernels=ref_hgm.GetFilterImpulseResponses(),
                                                                                                              filter_order=4, Print=True, max_iterations=10)
    print coeff.coefficients
    print coeff.frequencies
    assert len(curve_tracing_filters_iir) == len(coeff.coefficients) == len(coeff.frequencies)

test_sequenceofbiquads()