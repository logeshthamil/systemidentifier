...
def test_calculate_energy_of_filterkernels():
    branches = 5
    nl_system = nlsp.HammersteinModel(nonlinear_function=nlsp.nonlinear_function.HardClip(clipping_threshold=[-0.8,0.8]),
    ...
    system_identification = nlsp.system_identification.CosineSweep(select_branches=range(1,branches+1),excitation_length=2**18)
